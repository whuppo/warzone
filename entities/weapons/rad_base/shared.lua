...
function SWEP:GetViewModelPosition( pos, ang )
    ...
    if ( self.Owner:KeyDown( IN_SPEED ) and self.Owner:GetVelocity():Length() > 0 ) or self.HolsterMode then
...
function SWEP:Reload()
	...
	if self.Weapon:Clip1() == self.Primary.ClipSize or self.Owner:GetAmmoCount( self.Weapon:GetPrimaryAmmoType() ) <= 0 or self.HolsterMode or self.ReloadTime then return end
...
function SWEP:ReloadThink()
	...
	if self.ReloadTime and self.ReloadTime <= CurTime() then
		...
		self.ReloadTime = nil
		if self.Owner:GetAmmoCount( self.Weapon:GetPrimaryAmmoType() ) >= ( self.Primary.ClipSize - self.Weapon:Clip1() ) then
			self.Owner:SetAmmo( self.Owner:GetAmmoCount( self.Weapon:GetPrimaryAmmoType() ) - self.Primary.ClipSize + self.Weapon:Clip1(), self.Weapon:GetPrimaryAmmoType() )
			self.Weapon:SetClip1( self.Primary.ClipSize )
		elseif self.Owner:GetAmmoCount( self.Weapon:GetPrimaryAmmoType() ) < ( self.Primary.ClipSize - self.Weapon:Clip1() ) then
			self.Weapon:SetClip1( self.Weapon:Clip1() + self.Owner:GetAmmoCount( self.Weapon:GetPrimaryAmmoType() ) )
			self.Owner:SetAmmo( 0, self.Weapon:GetPrimaryAmmoType() )
		end
...
function SWEP:CanSecondaryAttack()
	...
	if self.HolsterMode or self.Owner:KeyDown( IN_SPEED ) then return false end
...
function SWEP:CanPrimaryAttack()
	...
	if self.HolsterMode or self.ReloadTime then return false end
	
	if self.Owner:GetAmmoCount( self.Weapon:GetPrimaryAmmoType() ) <= 0 and self.Weapon:Clip1() <= 0 then 
